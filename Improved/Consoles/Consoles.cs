...
namespace Improved.Consoles {
    public class CustomIo { //TODO: fix > 255 char stuff
        public static string ReadLine(Dictionary<int,string> maxPerWord,bool lettersNeeded = true,int minLength = 1,int maxLength = 255) { //minperword? min/max length?
            var x = CursorLeft;
            var y = CursorTop;
            var s = "";
            while(true) {
                ...
                if(k=='\b'&&s.Length>0) {
                    ...
                    if(CursorLeft==0) { CursorTop--; CursorLeft=WindowWidth; }
                    else CursorLeft--;
                    continue;
                }
                ...
                Write(k);
                switch(k) {
                    case '\r': case '\n': if(s.Length==s.Length.EnsureBetween(minLength,maxLength)) return s; break;
                    default: s+=k; break;
                }
            }
        }
    ...
    public static class Lists {
        ...
        public static ConcurrentDictionary<TKey,TValue> CDict<TKey, TValue>
            (params KeyValuePair<TKey,TValue>[] i) { return new ConcurrentDictionary<TKey,TValue>(i); }
        ...
        public static Tuple<T1,T2,T3> Tuple<T1, T2, T3>(T1 i1,T2 i2,T3 i3) {
        ...
        public static Tuple<T1,T2,T3,T4,T5,T6,T7,TRest> Tuple<T1, T2, T3, T4, T5, T6, T7, TRest>(T1 i1,T2 i2,T3 i3,T4 i4,T5 i5,T6 i6,T7 i7,TRest iRest) {
    ...
    public static class Consoles {
        ...
        public static void Initiate(int arrayLength,int lastLine) {
            ...
            Option=1; Page=-1; PagePlus=1; Index=1; Input=UpArrow;
        ...
        public static void CWrite(this string s,params object[] o) { Clear(); Write(s,o); }
        ...
        public static void WriteAt(this object o,int left,int top,int limit = -1) {
            SetCursorPosition(left,top); Write(o.ToString().PadRight(
               (limit<0||limit>=WindowWidth) ? WindowWidth-left-1 : limit));
        }
        ...
        public static int Choose(KeyMenu[] keyMenus, params string[] options) {
            ...
            while(true) {
                ...
                while(true) {
                    var oldIndex = Index;
                    switch(Input) {
                        case UpArrow: Option--; Index--; break;
                        case DownArrow: Option++; Index++; break;
                        ...
                        case Enter: return Option;
                        case default(ConsoleKey): break;
                        default:
                            int index;
                            if(keyMenus==null) { Input=ReadKey(true).Key; continue; }
                            //TODO: de-null check
                            if((index=keyMenus.ToList().FindIndex(k => k.Key==Input))!=-1) { Continue=true; return -index; }
        ...
        /// <summary>
        /// Moves words at the end of a line to the next line
        /// </summary>
        ...
        public static List<string> WordWrap(string paragraph,out int numberOfLines,int width = int.MaxValue,bool wordWrap = true,bool trim = true) {
        ...
        public static Item[] Spend(string[] text,Item[] items,int currency,out int currencyLeft,string singular = null,string textZero = null) {
            ...
            var maxCurrencyLength = Math.Max(
                currency.ToString().Length+text[0].Length+text[1].Length+text[2].Length+Math.Max(0,(singular=singular??text[1].RemoveLast()).Length-text[1].Length),
                textZero?.Length??0);
            var line = Top+1; var totalItems = items.ToArray(); var amtIsChanged = false;
            while(true) {
                switch(Input) {
                    case UpArrow: Option--; Index--; break;
                    case DownArrow: Option++; Index++; break;
                    case LeftArrow:
                        if(totalItems[Option].Amount-items[Option].Amount>0) { totalItems[Option].Amount--; currency+=items[Option].Cost; amtIsChanged=true; }
                        else if(sellIsEnabled&&totalItems[Option].Amount>0) { totalItems[Option].Amount--; currency+=items[Option].Value; amtIsChanged=true; }
                        break;
                    case RightArrow:
                        if(currency-items[Option].Cost>=0) { totalItems[Option].Amount++; currency-=items[Option].Cost; amtIsChanged=true; }
                        else if(sellIsEnabled&&currency-items[Option].Value>=0&&totalItems[Option].Amount-items[Option].Amount<0) { totalItems[Option].Amount++; currency-=items[Option].Value; amtIsChanged=true; }
                        break;
                    //TODO: test
                    case Enter: currencyLeft=currency; return totalItems;
                ...
                if(amtIsChanged) {
                    ...
                    if(currency!=0||textZero==null) s=text[0]+s+text[2]; //TODO: simplify
                    s.WriteAt(0,Top+MaxLines+2,maxCurrencyLength);
                }
                Maths.RMod(ref Option,TotalOptions);
                if(ScrollIsEnabled) //TODO: draw on first run if scrollisdisabled
                {
                ...
                if(Input==UpArrow||Input==DownArrow) { ' '.WriteAt(Left,line); '>'.WriteAt(Left,line=Index+Top+1); }
                Input=ReadKey(true).Key;
                amtIsChanged=false;
                PagePlus=0;
            }
